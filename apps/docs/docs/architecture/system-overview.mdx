...
| `subscribers/*.ts` | Event subscribers with metadata (`event`, `persistent`, optional `sync` for in-pipeline lifecycle). |
| `data/guards.ts` | Mutation guards with priority ordering, payload modification, and `afterSuccess` callbacks (UMES M). |
| `commands/interceptors.ts` | Command interceptors with `beforeExecute`/`afterExecute` hooks for execute and undo flows (UMES M). |
...
5. **Events & subscribers** – Writes first run sync subscribers and mutation guards inline in the request pipeline, then emit events that persistent subscribers process asynchronously (for example, index rebuilds, notifications).
