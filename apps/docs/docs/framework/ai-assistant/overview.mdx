...
| `OM_AI_<MODULE>_PROVIDER` | unset | Per-module provider override (Phase 1). Uppercased from the `moduleId`. Examples: `OM_AI_CATALOG_PROVIDER=openai`, `OM_AI_INBOX_OPS_PROVIDER=anthropic`. The legacy `<MODULE>_AI_PROVIDER` form is read as a backward-compatibility fallback. A provider-only preference can fall through when unconfigured; a paired provider/model override fails instead of sending that model to another provider. |
| `OM_AI_PROVIDER` | `openai` (when an `OPENAI_API_KEY` is set) | Process-wide default provider id. Built-in ids: `openai`, `anthropic`, `google`, `deepinfra`, `groq`, `together`, `fireworks`, `azure`, `litellm`, `ollama`, `openrouter`, `lm-studio`. A provider-only preference can fall through when unconfigured; when paired with `OM_AI_MODEL`, the provider must be configured. Legacy `OPENCODE_PROVIDER` is read as a backward-compatibility fallback. |
