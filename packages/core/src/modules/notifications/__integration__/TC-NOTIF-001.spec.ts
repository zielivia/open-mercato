...
test.describe('TC-NOTIF-001: Notification inbox lifecycle APIs', () => {
  test('should list, read, dismiss, restore, and mark notifications as read', async ({ request }) => {
    ...
    const notificationId = await createNotificationFixture(request, token, {
      type,
      title,
    ...
    const markAllReadBody = await readJsonResponse<{ ok?: boolean; count?: number }>(markAllReadResponse)
    ...
    const unreadCountBody = await readJsonResponse<{ unreadCount?: number }>(unreadCountResponse)
