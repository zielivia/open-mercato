...
description: Step-by-step guide for configuring Microsoft Entra ID (formerly Azure AD) as the OIDC and SCIM identity provider for Open Mercato SSO.
---
...
:::tip Prerequisite
You need a SCIM bearer token from Open Mercato. Generate one via the admin UI (SSO config -> Provisioning tab -> Generate Token) or the API (`POST /api/sso/scim/tokens` with the SSO config ID).
:::
