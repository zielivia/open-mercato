...
- **Native AI SDK escape hatch** (`generateText` callback on `runAiAgentText`, `generateObject` callback on `runAiAgentObject`): wrapper-preserved hook that runs `checkAgentPolicy`, `createModelFactory`, tool resolution, system-prompt composition, and the mutation-approval contract, then hands the prepared `{ model, tools, system, messages, maxSteps, output? }` to a caller-supplied callback so it can invoke `generateText` / `streamText` (text runtime) or `generateObject` / `streamObject` (object runtime) directly. Each runtime exposes a single override option named after the AI SDK function it ultimately calls (`generateText` and `generateObject` respectively) so the call site stays coherent with the AI SDK; the streaming variants are reached from inside the same callback. Required for callers that need raw AI SDK options (`providerOptions`, `experimental_telemetry`, `onStepFinish`, custom `abortSignal`, telemetry routing) without dropping policy or approval enforcement. Must document explicitly which guardrails still apply when the callback ignores supplied `tools` / `system` / `messages` (model factory and feature gate remain; whitelisting, prompt composition, and mutation approvals are forfeited along with whichever inputs the callback discards). Companion docs requirement: keep the "Using the Vercel AI SDK natively" section in `apps/docs/docs/framework/ai-assistant/agents.mdx` aligned with the implementation, including the Option A (raw AI SDK + `createModelFactory`) escape and the loss list (policy, mutation approvals, page-context resolution, tenant prompt/model overrides, attachment-to-model bridging, `<AiChat>` UI parts).
...
- Clarified that the streaming variants (`streamText` / `streamObject`) are reached from inside the same callback — the option name describes which runtime family is being overridden, not which exact AI SDK function the callback must call.
- No BC impact: nothing has been implemented under the previous `sdkOverride` name, so the rename happens before any caller adopts it. The loss surface (policy, mutation approvals, page-context resolution, tenant prompt/model overrides, attachment-to-model bridging, `<AiChat>` UI parts when streaming bypasses default UI parts) is unchanged.

### 2026-04-27 — Native Vercel AI SDK escape hatch (Phase 4 follow-up)
- Added a Phase 4 follow-up entry for a wrapper-preserved native AI SDK callback on `runAiAgentText` / `runAiAgentObject` (originally proposed as `sdkOverride`, later renamed to `generateText` / `generateObject` for AI SDK coherence — see 2026-04-28 entry) so callers can invoke `generateText` / `generateObject` / `streamText` / `streamObject` directly while the wrapper still runs `checkAgentPolicy`, `createModelFactory`, tool resolution, system-prompt composition, and the D16 mutation-approval contract.
