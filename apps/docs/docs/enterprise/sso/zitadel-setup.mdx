---
title: Zitadel Setup
description: Step-by-step guide for configuring Zitadel as the OIDC and SCIM identity provider for Open Mercato SSO.
---
...
:::tip Prerequisite
Generate a SCIM bearer token from Open Mercato via the admin UI (SSO config -> Provisioning tab -> Generate Token).
:::
